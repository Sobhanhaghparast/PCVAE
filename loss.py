...
def _earth_mover_distance_single_direction(
    x,
    y,
    x_lengths,
    y_lengths,
    ...
    for i in range(N):
        cost_matrix = torch.cdist(x[i, :x_lengths[i]], y[i, :y_lengths[i]], p=2).cpu().detach().numpy()
        row_ind, col_ind = linear_sum_assignment(cost_matrix)
        emd_distances.append(cost_matrix[row_ind, col_ind].sum())

    emd_distances = torch.tensor(emd_distances, device=x.device)

    if point_reduction == "mean":
        emd_distances /= x_lengths

    if batch_reduction == "mean":
        emd_distances = emd_distances.mean()
    elif batch_reduction == "sum":
        emd_distances = emd_distances.sum()
...
def earth_mover_distance(
    ...
    """
    Earth Mover's Distance (EMD) between two pointclouds x and y.

    Args:
        x: FloatTensor of shape (N, P1, D) representing a batch of point clouds with at most P1 points in each batch element,
            batch size N, and feature dimension D.
        y: FloatTensor of shape (N, P2, D) representing a batch of point clouds with at most P2 points in each batch element,
            batch size N, and feature dimension D.
        ...
        point_reduction: Reduction operation to apply for the loss across the points, can be one of ["mean", "sum"] or None.

    Returns:
        loss: Tensor giving the reduced Earth Mover's Distance between the pointclouds in x and y.
    """
    x, x_lengths, _ = _handle_pointcloud_input(x, x_lengths, None)
    y, y_lengths, _ = _handle_pointcloud_input(y, y_lengths, None)

    emd_x = _earth_mover_distance_single_direction(
        x,
        y,
        x_lengths,
        y_lengths,
